...
* `installer/ui.sh` exports `TERM=linux` when the variable is empty.
...
* Every string passed to dialog is run through `strip_ansi()`, which removes
  CSI (`ESC[…`), OSC (`ESC]…`), and other VT escape sequences so that
  coloured output from system commands (e.g. `lsblk`, `sbctl`) is never
